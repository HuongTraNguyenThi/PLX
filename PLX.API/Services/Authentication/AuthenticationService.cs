...
namespace PLX.API.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<APIResponse> GenerateOTP(OTPDTO oTPDTO)
        {
            ...
            List<OTP> otpRecord = await _otpRepository.FindOTPByPhoneAndActive(oTPDTO.Phone, oTPDTO.Active);
        ...
        public async Task<APIResponse> ValidateOTP(string phone, int otp)
        {
            int value = 3;
            var otpRecord = await _otpRepository.FindOTPByPhone(phone);
            TimeSpan ts = DateTime.Now - otpRecord.CreateTime11;
            if (ts.Minutes <= value && otpRecord.Active == true)
